...
class SeedVR2:
    ...
    def INPUT_TYPES(cls) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "required": {
                "images": ("IMAGE", ),
                "model": (folder_paths.get_filename_list("seedvr2"),),
    ...
    def execute(self, images: torch.Tensor, model: str, vae: str, seed: int, target_short_side: int, 
        ...
        cfg_scale = 1.0
        try:
            model_path = folder_paths.get_full_path("seedvr2", model)
            vae_path = folder_paths.get_full_path("vae", vae)
            return self._internal_execute(images, model_path, vae_path, seed, target_short_side, cfg_scale, batch_size, preserve_vram, temporal_overlap, debug, block_swap_config)
    ...
    def _internal_execute(self, images, model_path, vae_path, seed, target_short_side, cfg_scale, batch_size, preserve_vram, temporal_overlap, debug, block_swap_config):
        ...
        self.runner = configure_runner(
            model_path, vae_path, preserve_vram, debug, 
            block_swap_config=block_swap_config,
        ...
        sample = generation_loop(
            self.runner, images, cfg_scale, seed, target_short_side, 
