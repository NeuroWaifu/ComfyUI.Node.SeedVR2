...
def configure_runner(model_path, vae_path, preserve_vram=False, debug=False, block_swap_config=None, cached_runner=None):
    """
    Configure and create a VideoDiffusionInfer runner for the specified model
    
    Args:
        model_path (str): Model file path
        vae_path (str): VAE file path
    ...
    config.vae.checkpoint = os.path.basename(vae_path)
    ...
    # Configure models
    t = time.time()
    runner = configure_dit_model_inference(runner, device, model_path, config, preserve_vram, model_weight, vram_info, debug, block_swap_config)
    if debug:
        print(f"🔄 RUNNER : DIT MODEL INFERENCE TIME: {time.time() - t} seconds")
    
    t = time.time()
    runner = configure_vae_model_inference(runner, device, vae_path, config, preserve_vram, model_weight, vram_info, debug, block_swap_config)
    if debug:
